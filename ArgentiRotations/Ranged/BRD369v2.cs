...
public sealed class Brd369V2 : BardRotation
{
    ...
    protected override IAction? CountDownAction(float remainTime)
    {
        ...
        if (EnablePrepullHeartbreakShot && remainTime <= 1.6f && HeartbreakShotPvE.CanUse(out var act)) return act;
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if (IronJawsPvE.CanUse(out act, skipStatusProvideCheck: true) && IronJawsPvE.Target.Target.WillStatusEnd(30, true, IronJawsPvE.Setting.TargetStatusProvide ?? []))
            if (Player.HasStatus(true, StatusID.BattleVoice, StatusID.RadiantFinale, StatusID.RagingStrikes) &&
        ...
        if (RadiantEncorePvE.CanUse(out act, skipComboCheck: true))
            if (InBurstStatus && Player.HasStatus(true, StatusID.RagingStrikes))
                return true;
        ...
        if (BlastArrowPvE.CanUse(out act))
        {
            ...
            if (HostileTarget?.WillStatusEndGCD(1, 0.5f, true, StatusID.Windbite, StatusID.Stormbite,
                    StatusID.VenomousBite, StatusID.CausticBite) ?? false) return false;
        }
        ...
        {
            // Do not use WindbitePvE or VenomousBitePvE if both statuses are present and IronJawsPvE has enough levels
        }
        else
        {
    ...
    protected override bool EmergencyAbility(IAction nextGcd, out IAction? act)
    {
        ...
        {
            if (((EmpyrealArrowPvE.Cooldown.IsCoolingDown && !EmpyrealArrowPvE.Cooldown.WillHaveOneChargeGCD(1)) ||
                 !EmpyrealArrowPvE.EnoughLevel) && Repertoire != 3)
    ...
    protected override bool AttackAbility(IAction nextGcd, out IAction? act)
    {
        ...
        if (Song == Song.Wanderer)
        {
            ...
            if (_inBurstStatusCount < 1)
                ...
                {
                    if (PotionTimings is PotionTimingOption.ZeroAndSixMins or PotionTimingOption.ZeroFiveAndTenMins &&
            ...
            if (EnablePrepullHeartbreakShot)
            {
                if (_inBurstStatusCount >= 1)
                {
                    ...
                    {
                        if (_inBurstStatusCount == 1 && PotionTimings == PotionTimingOption.TwoAndEightMins &&
                            UseBurstMedicine(out act)) return true;
                        if (_inBurstStatusCount == 3 && PotionTimings == PotionTimingOption.ZeroAndSixMins &&
                            UseBurstMedicine(out act)) return true;
                        if (_inBurstStatusCount == 4 && PotionTimings == PotionTimingOption.TwoAndEightMins &&
                            UseBurstMedicine(out act)) return true;
                    }

                    if (TheWanderersMinuetPvE.Cooldown.IsCoolingDown && TheWanderersMinuetPvE.Cooldown.ElapsedAfter(
                                                                         2.01f)
            ...
            else if (!EnablePrepullHeartbreakShot)
            {
                if (_inBurstStatusCount >= 1)
                {
                    if (TheWanderersMinuetPvE.Cooldown.IsCoolingDown && TheWanderersMinuetPvE.Cooldown.ElapsedAfterGCD(
                                                                         1)
                                                                     && RadiantFinalePvE.CanUse(out act)) return true;
        ...
        if (RadiantFinalePvE.EnoughLevel && RadiantFinalePvE.Cooldown.IsCoolingDown && BattleVoicePvE.EnoughLevel &&
            !BattleVoicePvE.Cooldown.IsCoolingDown) return false;
        ...
        if (EmpyrealArrowPvE.CanUse(out act))
        {
            ...
            else if (Song == Song.Army && WeaponRemain > 0.9f)
            {
                return true;
            }
        }
        ...
        if (PitchPerfectPvE.CanUse(out act))
        {
            ...
            if (Repertoire >= 2 && EmpyrealArrowPvE.Cooldown.WillHaveOneChargeGCD(0, 1.25f) &&
                RadiantFinalePvE.Cooldown.IsCoolingDown && RagingStrikesPvE.Cooldown.IsCoolingDown) return true;
        }

        if (_inBurstStatusCount == 2 && Song == Song.Wanderer &&
            PotionTimings == PotionTimingOption.ZeroFiveAndTenMins && SongEndAfter(5) &&
            UseBurstMedicine(out act)) return true;

        if (MagesBalladPvE.CanUse(out act) && InCombat && WeaponRemain < 1)
            if (Song == Song.Wanderer && SongEndAfter(WandRemainTime))
                return true;
        ...
        if (ArmysPaeonPvE.CanUse(out act) && InCombat)
        {
            ...
            else if (Song == Song.Mage && SongEndAfter(MageRemainTime) && _inBurstStatusCount > 1)
            {
                return true;
            }
        }
        ...
        if (SidewinderPvE.CanUse(out act))
        {
            ...
            if (Player.HasStatus(true, StatusID.BattleVoice) && Player.HasStatus(true, StatusID.RadiantFinale) &&
                RagingStrikesPvE.Cooldown.IsCoolingDown) return true;

            if (!BattleVoicePvE.Cooldown.WillHaveOneCharge(10) && !RadiantFinalePvE.Cooldown.WillHaveOneCharge(10) &&
                RagingStrikesPvE.Cooldown.IsCoolingDown) return true;
        ...
        // Logic to ensure 4 HeartbreakShots in a burst window
        ...
        return BetterBloodletterLogic(out act) || base.AttackAbility(nextGcd, out act);
    }
    ...
    private bool CanUseApexArrow(out IAction act)
    {
        ...
        if (HostileTarget?.WillStatusEndGCD(1, 1, true, StatusID.Windbite, StatusID.Stormbite, StatusID.VenomousBite,
                StatusID.CausticBite) ?? false) return false;

        if (SoulVoice >= 80 && Player.HasStatus(true, StatusID.RagingStrikes) &&
            Player.WillStatusEnd(10, false, StatusID.RagingStrikes)) return true;

        if (SoulVoice == 100 && Player.HasStatus(true, StatusID.RagingStrikes) &&
            Player.HasStatus(true, StatusID.BattleVoice)) return true;
    ...
    {
        var isMedicated = Player.HasStatus(true, StatusID.Medicated);
    ...
    private static bool InBurstStatus => Player.HasStatus(true, StatusID.RagingStrikes) &&
                                         Player.HasStatus(true, StatusID.BattleVoice) &&
                                         Player.HasStatus(true, StatusID.RadiantFinale);

    private static int _inBurstStatusCount;
